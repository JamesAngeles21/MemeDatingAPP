...
from TestConstants import TEST_CREDENTIALS, TEST_CREDENTIALS2, TEST_ACCOUNT, TEST_ACCOUNT2, TEST_CONVERSATION_BETWEEN, PROFILE_BASE_URL, TEST_CONVERSATION, TEST_CONVERSATION2, CONVERSATION_BASE_URL, CONVERSATION_USERNAME_URL, CONVERSATION_BETWEEN_USER_URL, TEST_CONVERSATION_DELETED, CONVERSATION_BATCH_GET_URL, TEST_CONVERSATION3, TEST_ACCOUNT3, TEST_CREDENTIALS3, MATCHES_BASE_URL, TEST_MATCH, TEST_MATCH_ANOTHER
...
class ConversationsTests(APITestCase):
    ...
    def test_retrive_empty_message(self):
        response = self.client.post(PROFILE_BASE_URL, TEST_ACCOUNT, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        response = self.client.post(PROFILE_BASE_URL, TEST_ACCOUNT2, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_get_all_message(self):
        ...
        response = self.client.post(MATCHES_BASE_URL, TEST_MATCH_ANOTHER, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(Matches.objects.count(), 4)

        response = self.client.get(CONVERSATION_USERNAME_URL)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 2)
        self.assertConversationMessages(response.data[0], TEST_CONVERSATION3)
        self.assertConversationMessages(response.data[1], TEST_CONVERSATION2)
